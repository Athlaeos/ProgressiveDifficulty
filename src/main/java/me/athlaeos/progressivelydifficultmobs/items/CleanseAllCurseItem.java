...
public class CleanseAllCurseItem extends ActiveItem {
    private final int cooldown;
    YamlConfiguration config;

    public CleanseAllCurseItem(){
        key = new NamespacedKey(Main.getInstance(), "pdm-cleanse_all_curse");
        config = ConfigManager.getInstance().getConfig("items.yml").get();
        curseLimit = config.getInt("cleanse_all_curse_item.curse_limit");
        useRecipe = config.getBoolean("cleanse_all_curse_item.use_recipe");
        cooldown = config.getInt("cleanse_all_curse_item.cooldown");

        Material m;
        try {
            m = Material.valueOf(config.getString("cleanse_all_curse_item.item.material"));
        ...
        ItemStack item = Utils.createItemStack(
                m,
                Utils.chat(config.getString("cleanse_all_curse_item.item.name")),
                config.getStringList("cleanse_all_curse_item.item.lore"));
        this.item = item;
    ...
    private void createRecipe(){
        ...
        List<String> shape = config.getStringList("cleanse_all_curse_item.recipe.shape");
        try {
        ...
        for (char c : materials){
            if (config.getString("cleanse_all_curse_item.recipe." + c) == null){
                System.out.println("Error: Cleanse all curse item. Material " + c + " in the shape grid was not defined after, cancelled crafting recipe");
                return;
            } else {
                try {
                    r.setIngredient(c, Material.valueOf(config.getString("cleanse_all_curse_item.recipe." + c)));
                } catch (IllegalArgumentException e){
                    System.out.println("Invalid crafting material for the cleanse all curse item at " + c + ", cancelled crafting recipe");
                    return;
                }
            }
        }
    ...
    public void execute(Event e) {
        ...
        if (PlayerCurseManager.getInstance().getCurse(p.getUniqueId()) < curseLimit){
            if (CooldownManager.getInstance().cooldownLowerThanZero(p, "cleanse_all_curse_cooldown") || p.hasPermission("pdm.cancelitemcooldowns")){
                if (p.getGameMode() != GameMode.CREATIVE){
                    p.getInventory().getItemInMainHand().setAmount(p.getInventory().getItemInMainHand().getAmount() - 1);
                }
                if (!p.hasPermission("pdm.cancelitemcooldowns")){
                    CooldownManager.getInstance().setCooldown(p, cooldown * 1000, "cleanse_all_curse_cooldown");
                }
                PlayerCurseManager.getInstance().setCurse(p.getUniqueId(), 0);
                    ...
                    new BukkitRunnable(){
                        ...
                        public void run() {
                            if (timer < 3){
                                ...
                                p.playSound(p.getLocation(), Sound.ENTITY_WITHER_DEATH, 0.4F, 0.5F);
                                p.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 100, 0));
                                p.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 120, 3));
                ...
                p.sendMessage(Utils.chat(PluginConfigurationManager.getInstance().getPlayerCantUseItemError()
                        .replace("{duration}", String.format("%s", Utils.msToTimestamp(CooldownManager.getInstance().getCooldown(p, "cleanse_all_curse_cooldown"))))));
            }
        } else {
