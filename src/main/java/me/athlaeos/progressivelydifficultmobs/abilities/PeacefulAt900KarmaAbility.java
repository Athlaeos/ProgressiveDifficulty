...
public class PeacefulAt900KarmaAbility extends Ability {
    ...
    public PeacefulAt900KarmaAbility(){
        this.name = Utils.chat("&a&lPeaceful to 900+ karma");
        this.description = Arrays.asList(
                ...
                Utils.chat("&7with &a900&7 or more karma (good karma)"));
            ...
            if (!e.isCancelled()){
                if (e.getTarget() instanceof Player){
                    ...
                    if (p.getPersistentDataContainer().has(key, PersistentDataType.DOUBLE)){
                        if (p.getPersistentDataContainer().get(key, PersistentDataType.DOUBLE) >= 900){
