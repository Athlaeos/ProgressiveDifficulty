...
public class GiveOffhandAt600BadKarmaAbility extends Ability {
    ...
    public GiveOffhandAt600BadKarmaAbility(){
        this.name = Utils.chat("&c&lGive off-hand item to 600- karma");
        this.description = Arrays.asList(
                ...
                Utils.chat("&7-600 or less karma when"),
    ...
    public void execute(Entity entity, Player player, Event event) {
        if (entity instanceof LivingEntity){
            ...
            if (player.getPersistentDataContainer().has(key, PersistentDataType.DOUBLE)){
                if (player.getPersistentDataContainer().get(key, PersistentDataType.DOUBLE) <= -600){
