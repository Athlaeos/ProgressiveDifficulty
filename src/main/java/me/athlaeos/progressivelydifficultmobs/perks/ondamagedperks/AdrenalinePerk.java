...
public class AdrenalinePerk extends Perk {
    private int cooldown;
    private int duration;
    private int amplifier;
    public AdrenalinePerk(){
        this.id = 2047;
        this.icon = Material.SUGAR;
        ...
        this.description = Utils.seperateStringIntoLines(Utils.chat("&7Players with this perk" +
                " receive a speed buff upon being damaged by an entity"), 36, "&7");
    ...
    public void execute(Event e) {
        if (e instanceof EntityDamageByEntityEvent){
            ...
            if (!event.isCancelled()){
                ...
                if (WorldguardManager.getWorldguardManager().isLocationInRegionWithFlag(defender.getLocation(), "pdm-adrenaline-deny")
                ...
                if (defender instanceof Player){
                    ...
                    if (p.hasPotionEffect(PotionEffectType.SPEED)){
                        if (p.getPotionEffect(PotionEffectType.SPEED).getAmplifier() > amplifier){
                            return;
                        }
                    }
                    if (CooldownManager.getInstance().cooldownLowerThanZero(p, "perks_adrenaline_cooldown")){
                        ...
                        if (!potionEvent.isCancelled()){
                            ...
                            CooldownManager.getInstance().setCooldown(p, cooldown * 1000, "perks_adrenaline_cooldown");
                        }
                    }
                }
            }
